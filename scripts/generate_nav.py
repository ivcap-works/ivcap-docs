...
def build_capabilities_nav(examples_dir: Path, docs_root: Path, capabilities_dir: Path) -> list:
    """
...
def main() -> None:
    ...
    config["nav"] = replace_placeholder(config["nav"],          "__CAPABILITIES__", capabilities_nav)
    config["nav"] = replace_placeholder(config["nav"],          "__EXAMPLES__",     examples_nav)

    output.write_text(yaml.dump(config, allow_unicode=True, sort_keys=False))
